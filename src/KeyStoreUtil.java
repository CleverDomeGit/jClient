...
public class KeyStoreUtil {
	...
	public static String spillBeans(java.security.cert.Certificate cert)
			throws GeneralSecurityException {
		StringBuffer buffer = new StringBuffer("Certificate type: "
				+ cert.getType() + endLine + "Encoded data: " + endLine);
	...
	/**
	 * As an application, this class will operate something like the <b>keytool
	 * -list</b> command: it will read out every alias in the keystore, and with
	 * the user's provided password for a given key will write out all data on
	...
	public static void main(String[] args) throws Exception {
		...
		while (each.hasNext())
			try {
				...
				Key key = getKey(keystore, alias,
						getUserInput("Key password: "));
				System.out.println(spillBeans(key));

				java.security.cert.Certificate certificate = getCertificate(
						keystore, alias);
